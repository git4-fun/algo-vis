...
function createNewArray(length) {
  ...
  for (let i = 0; i < length; i++) {
    randomArray.push(
      Math.floor(Math.random() * (parseInt(barsElem.clientHeight) - 1) + 1)
    );
  }
  return randomArray;
}
  ...
  for (let i = 0; i < array.length - 1; i++) {
    for (let j = 0; j < array.length - 1 - i; j++) {
      if (array[j] > array[j + 1]) {
        barsElem.querySelector(`#index_${j}`).style.borderColor =
          colorCode.yellow;
        await delay(1 / parseInt(document.querySelector("#speed").value));
        swapElements(
          barsElem.querySelector(`#index_${j}`),
          barsElem.querySelector(`#index_${j + 1}`)
        );
        swap(array, j + 1, j);
        await delay(1 / parseInt(document.querySelector("#speed").value));
      }
    ...
    barsElem.querySelector(`#index_${array.length - 1 - i}`).style.borderColor =
      colorCode.green;
  }
}
  ...
  for (let i = 0; i < array.length - 1; i++) {
    ...
    for (let j = i + 1; j < array.length; j++) {
      if (array[j] < array[iMin]) {
        iMin = j;
        document.querySelector(`#index_${iMin}`).style.borderColor =
          colorCode.darkBlue;
        await delay(1 / parseInt(document.querySelector("#speed").value));
        document.querySelector(`#index_${iMin}`).style.borderColor =
          colorCode.blue;
    ...
    if (iMin != i) {
      swap(array, iMin, i);
      swapElements(
        barsElem.querySelector(`#index_${iMin}`),
        barsElem.querySelector(`#index_${i}`)
      );
      document.querySelector(`#index_${iMin}`).style.borderColor =
        colorCode.blue;
      await delay(1 / parseInt(document.querySelector("#speed").value));
...
async function insertionSort(array) {
  ...
  for (let i = 1; i < array.length; i++) {
    ...
    while (j >= 0 && element < array[j]) {
      barsElem.querySelector(`#index_${j + 1}`).style.borderColor =
        colorCode.yellow;
      await delay(0.5 / parseInt(document.querySelector("#speed").value));
      swapElements(
        barsElem.querySelector(`#index_${j + 1}`),
...
async function merge(arr, l, m, h) {
  ...
  while (i <= m && j <= h) {
    if (arr[i] <= arr[j]) {
      document.querySelector(
        `#index_${l + k}`
      ).style.height = `${auxArray[i]}px`;
      document.querySelector(`#index_${l + k}`).style.borderColor =
        colorCode.green;
      await delay(1 / parseInt(document.querySelector("#speed").value));
      temp[k++] = arr[i++];
    } else {
  ...
  for (let i = l, k = 0; i <= h; i++, k++) arr[i] = temp[k];
}
...
async function partition(arr, l, h) {
  ...
  for (let j = i; j <= h; j++) {
    if (arr[j] <= arr[pivot]) {
      swapElements(
        document.querySelector(`#index_${j}`),
        document.querySelector(`#index_${i}`)
      );

      document.querySelector(`#index_${j}`).style.borderColor =
        colorCode.darkBlue;
      document.querySelector(`#index_${i}`).style.borderColor =
        colorCode.purple;
      await delay(1 / parseInt(document.querySelector("#speed").value));
      document.querySelector(`#index_${i}`).style.borderColor =
        colorCode.yellow;
  ...
  swapElements(
    document.querySelector(`#index_${pivot}`),
    document.querySelector(`#index_${i - 1}`)
  );
  swap(arr, pivot, i - 1);
  await delay(1 / parseInt(document.querySelector("#speed").value));
...
async function quickSort(arr, l, h) {
  ...
  infoBody.innerHTML = `
    ...
    <p>
        It selects a "pivot" element from the array and partitions the other elements into two sub-arrays according to whether they are less than or greater than the pivot. The process is recursively applied to the sub-arrays until the entire array is sorted.
    </p>
    
    <p>
        <b><i>Worst Case</i></b> Time Complexity: <b><i>O(n<sup>2</sup>)</i></b>. The worst-case time complexity of Quick Sort occurs when the pivot element consistently results in highly imbalanced partitions.
    </p>

    <p>
        <b><i>Average Case</i></b> Time Complexity: <b><i>O(n log n)</i></b>. On average, Quick Sort partitions the array into two sub-arrays of approximately equal size. 
    </p>

    <p>
        <b><i>Best Case</i></b> Time Complexity: <b><i>O(n)</i></b>. The best-case time complexity of Quick Sort occurs when the pivot element is chosen such that it partitions the array into two nearly equal-sized sub-arrays.
    </p>

    <div class="image-div">
        <img class="image-file" src="https://runestone.academy/ns/books/published/pythonds/_images/partitionA.png">
        <p>
            Source: <a href="https://runestone.academy/ns/books/published/pythonds/SortSearch/TheQuickSort.html" target="_blank">Runestone academy</a>
...
function leftChild(arr, i, h) {
  if (2 * (i + 1) <= h + 1) return 2 * (i + 1) - 1;
  return -1;
}

function rightChild(arr, i, h) {
  if (2 * (i + 1) + 1 <= h + 1) return 2 * (i + 1);
  return -1;
}
...
async function maxHeapify(arr, l, h) {
  for (let root = h; root >= l; root--) {
    if (leftChild(arr, root, h) != -1) {
      ...
      if (rc != -1) {
        if (arr[lc] < arr[rc]) max = rc;
      }

      if (arr[root] < arr[max]) {
        document.querySelector(`#index_${root}`).style.borderColor =
          colorCode.orange;
        document.querySelector(`#index_${max}`).style.borderColor =
          colorCode.yellow;
        await delay(0.5 / parseInt(document.querySelector("#speed").value));
        swapElements(
          barsElem.querySelector(`#index_${root}`),
          barsElem.querySelector(`#index_${max}`)
        );
        document.querySelector(`#index_${root}`).style.borderColor =
          colorCode.blue;
        document.querySelector(`#index_${max}`).style.borderColor =
...
async function heapSort(arr, h) {
  ...
  infoBody.innerHTML = `
    ...
    <p>
        It operates by building a max-heap (for ascending order) or min-heap (for descending order) from the input array and then repeatedly extracting the root element to create the sorted output.
    </p>
    
    <p>
        <b><i>Worst and Average Case</i></b> Time Complexity: <b><i>O(n log n)</i></b>. The algorithm performs log n operations for each of the n elements during the heapification process, leading to an overall time complexity of O(n log n).
    </p>

    <p>
        <b><i>Best Case</i></b> Time Complexity: <b><i>O(n)</i></b>. In the best-case scenario, where the input array is already a valid binary heap, the time complexity of Heap Sort reduces to O(n). 
    </p>

    <div class="image-div">
        <img class="image-file" src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/dc/Heapsort.svg/900px-Heapsort.svg.png">
        <p>
            Source: <a href="https://de.wikipedia.org/wiki/Heapsort" target="_blank">Wikipedia</a>
        </p>
    </div>
    `;
  while (h > 0) {
    ...
    await delay(0.5 / parseInt(document.querySelector("#speed").value));
    swapElements(
      barsElem.querySelector(`#index_${0}`),
      barsElem.querySelector(`#index_${h}`)
    );
